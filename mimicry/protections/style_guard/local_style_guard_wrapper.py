#!/usr/bin/env python3
"""Batch-friendly wrapper for StyleGuard protection generation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run StyleGuard on one image or one artist folder.",
    )
    ...
    parser.add_argument(
        "--tool-root",
        type=Path,
        default=Path("mimicry/protections/tools/style_guard"),
        help="Root path of StyleGuard checkout.",
    )

    # Shared model args
    add_model_args(parser)

    # StyleGuard specific args
    parser.add_argument(
        "--instance-prompt",
    ...
    # Style loss (off by default — standard PGD only)
    parser.add_argument(
        "--style-loss-weight",
        type=float,
        default=0.0,
        help="Weight for style transfer loss. 0 disables it (standard PGD).",
    )
    parser.add_argument(
        "--target-image-dir",
        type=Path,
...
def run_style_guard(
    ...
    """Run StyleGuard once for the given input folder."""
    ...
    # Style loss (off by default)
    if args.style_loss_weight > 0:
        command.extend(["--style_loss_weight", str(args.style_loss_weight)])
        if args.target_image_dir is not None:
            command.extend(["--target_image_dir", str(args.target_image_dir.resolve())])
...
def run_dir_mode(
    ...
    """Run StyleGuard for one artist folder."""
    ...
    with tempfile.TemporaryDirectory(prefix="styleguard_batch_") as temp_dir:
...
def run_file_mode(
    ...
    """Run StyleGuard for one image by wrapping into a temporary folder job."""
    with tempfile.TemporaryDirectory(prefix="styleguard_single_") as temp_dir:
...
def main() -> int:
    """Entry point for StyleGuard batch wrapper."""
    args = parse_args()
